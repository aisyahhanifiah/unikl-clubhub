...
@section('title', 'Edit ' .$meeting->name.' |')
                ...
                <div class="card bg-secondary shadow">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">{{ Breadcrumbs::render('committee.meeting.edit', $meeting->id) }}</h3>
                            </div>
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('committee.meeting.update', $meeting) }}" autocomplete="off">
                            {{ csrf_field() }}
                            {{ method_field('PUT') }}
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('datetime') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="datetime-local" name="datetime" id="input-datetime" class="form-control form-control-alternative{{ $errors->has('datetime') ? ' is-invalid' : '' }}" placeholder="{{ __('Start date') }}" value="{{ date('Y-m-d\TH:i',strtotime($meeting->datetime)) }}" required>
                                ...
                                <div class="form-group{{ $errors->has('venue') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="venue" id="input-venue" class="form-control form-control-alternative{{ $errors->has('venue') ? ' is-invalid' : '' }}" placeholder="{{ __('Venue') }}" value="{{ old('venue', $meeting->venue) }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('club') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control form-control-alternative{{ $errors->has('club') ? ' is-invalid' : '' }}" name="club_id" id="input-club">
                                            @foreach($clubs as $club)
                                                <option value="{{$club->id}}" {{ $meeting->club_id==$club->id ? "selected" : ''}}>{{$club->name}}</option>
                                            @endforeach
                                        </select>
